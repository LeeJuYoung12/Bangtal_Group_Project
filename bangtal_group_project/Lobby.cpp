...
ObjectID startbutton, endbutton, portalbutton[4];
SoundID introbgm;
TimerID lobby_update_t;
// 0가 왼쪽위, 1이 왼쪽아래, 2가 오른쪽아래, 3이 오른쪽위, 4가 가운데
int portalX[4] = { 600, 350, 450, 400};
int portalY[5] = { 60, 370, 200, 580};
//게임 위치확인 (함수꼬이지 않게 하는용) 0은 로비 1,2,3,4,5 는 게임
int nowgamenum = 0;

bool g1_cleared = false, g2_cleared = false, g3_cleared = false;
bool lobby_check1 = true, lobby_check2 = true, lobby_check3 = true;
...
void lobby_timerCallback(TimerID timer) {
	if (timer == lobby_update_t) {
		...
		if (g2_cleared&& lobby_check2) {
			lobby_check2 = false;
			setObjectImage(portalbutton[2], "image/portal.png");
		}
		if (g3_cleared&& lobby_check3) {
			lobby_check3 = false;
